...
export default function DetailerTaskDetailScreen() {
  ...
  // optional for final (kept, but only shown on detail_final)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // if we have vehicle -> try load inspection + items
      if (res.vehicle?.id) {
        const it = inspectionTypeFromTaskType(res.task.type);
        const ins = await fetchLatestInspection(res.vehicle.id, it);
        setInspection(ins);

        if (ins?.id) {
          const items = await fetchInspectionItems(ins.id);

          const mappedSaved: DamageEntry[] = items
          ...
          setSavedEntries(mappedSaved);

          // if task not done, do NOT mix saved entries into editable draft
          // (editing DB rows would require update/delete APIs)
          if (res.task.status === "open") setDraftEntries([]);
        }
      }
    } catch (e: any) {
  ...
  const openAdd = useCallback((kind: DamageKind) => {
    setEditOpen({ kind });
    setPosSelected([]);
    setCommentInput("");
    setPhotosInput([]);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.hintRow}>
          ...
          <Text style={styles.hintText}>
            Bitte überprüfe das Fahrzeug für die Übergabe und dokumentiere die
            relevanten Punkte.
          </Text>
        </View>
          ...
          >
            <View style={styles.addHeader}>
              <Text style={styles.addTitle}>
                {editOpen
                  ? `${damageLabel(editOpen.kind)} ${editOpen.entryId ? "bearbeiten" : "hinzufügen"}`
                  : "Bearbeiten"}
              </Text>
